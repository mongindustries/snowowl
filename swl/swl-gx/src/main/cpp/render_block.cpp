...
render_block& render_block::operator= (render_block&&) noexcept = default;
...
render_block_scope::render_block_scope  (render_block& block, render_pipeline& pipeline): block(&block) {
  block.reset(pipeline);
}

render_block_scope::~render_block_scope () {
  block.get().close();
}
